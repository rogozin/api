...
* [Готовность резюме к публикации](#ready_to_publish)
...
`progress`:
 ...
 Поле | Тип | Описание
 ---- | ----| --------
 percentage | число | Процент заполненности резюме
 ready_to_publish | логический | Можно ли публиковать резюме
 publish_url | строка | Url для публикации резюме
 mandatory | список | Список полей, которые обязательны, но еще не заполнены
 mandatory[].id | строка | Id поля
 mandatory[].name | строка | Название поля
 recommended | список | Список полей, которые рекомендованы к заполнению, но ещё не заполнены
 recommended[].id | строка | Id поля
 recommended[].name | строка | Название поля
...
<a name="ready_to_publish"></a>
## Готовность резюме к публикации

Метод возвращает [информацию о готовности резюме к публикации](#author-progress) (в том числе процент заполненности резюме, обязательные и рекомендуемые к заполнению поля) и [замечания модератора](#author-moderation-notes) для выбранного резюме. Поля аналогичны таковым в списке [дополнительных полей для автора резюме](##additional-author-fields).

```
GET /resumes/{resume_id}/ready_to_publish
```
...
`progress.mandatory` станет пустым списком, резюме можно будет
