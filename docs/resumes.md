...
элемент справочника [resume_access_type](dictionaries.md).
 ...
 whitelist | Резюме не видно никому, кроме указанных компаний. Если откликнуться на вакансию компании, который не входит в список, компания автоматически в него попадёт. См. [списки видимости резюме](#visibility_lists).
...
для которых резюме должно быть видимо или скрыто. Для этих типов в поле `access` добавляется ссылка на список
(ключ `list_url`):
...
```json
{
    "access": {
        "type": {
            ...
            "list_url": "https://api.hh.ru/resumes/0123456789abcdef/whitelist"
        }
    }
}
```
