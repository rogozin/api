...
Каждый элемент из `items` обладает следующими полями:
 ...
 allowed_permissions | список объектов | список доступных прав
...
```json
 ...
 permissions | список объект | [права менеджера](#dict)
...
Изменять можно только поля приведенные в json. Описание полей можно посмотреть в [запросе на создание](#add).
...
Удаление менеджера происходитт не сразу, это может занять какое-то время. 
